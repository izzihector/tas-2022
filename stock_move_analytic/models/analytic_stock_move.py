...
class ASMStockLocation(models.Model):
    _inherit = 'stock.location'

    valuation_analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account')
...
class StockMove(models.Model):
    ...
    def _get_analytic_acc_from_move(self):
        ...
        if not analytic_acc and self.location_dest_id.valuation_in_account_id:
            analytic_acc = self.location_dest_id.valuation_analytic_account_id.id
        return analytic_acc

# pylint: disable=R0913
    def _prepare_account_move_line(self, qty, cost, credit_account_id,
                                   debit_account_id, description):
        res = super(StockMove, self)._prepare_account_move_line(qty, cost, credit_account_id,
                                                                debit_account_id, description)
        if res and res[0] and res[0][2]:
            analytic_acc = self._get_analytic_acc_from_move()
            for line in res:
                if line[2]['debit'] > 0:
                    line[2]['analytic_account_id'] = analytic_acc
                    break
        return res
